...
describe('helix-admin', () => {
  ...
  describe('getAdminUrl', () => {
    it('creates a correct URL with all parameters', () => {
      ...
      expect(url.toString()).to.equal(`https://admin.${process.env.HLX_PROD_SERVER_HOST_PAGE}/preview/adobe/aem-boilerplate/main/path/to/resource`);
    });
    ...
    it('creates a correct URL with specified ref', () => {
      ...
      expect(url.toString()).to.equal(`https://admin.${process.env.HLX_PROD_SERVER_HOST_PAGE}/preview/adobe/aem-boilerplate/foo/path/to/resource`);
    });
    ...
    it('creates discover URL correctly', () => {
      ...
      expect(url.toString()).to.equal(`https://admin.${process.env.HLX_PROD_SERVER_HOST_PAGE}/discover?url=https%3A%2F%2Fexample.com`);
    });
    ...
    describe('API v2', () => {
      it('creates correct URL for login endpoint', () => {
        ...
        expect(url.toString()).to.equal(`https://api.${process.env.HLX_PROD_SERVER_HOST_LIVE}/login?org=adobe&site=aem-boilerplate`);
      });

      it('creates correct URL for logout endpoint', () => {
        const config = { ...siteStore, apiUpgrade: true };
        const url = createAdminUrl(config, 'logout');
        expect(url.toString()).to.equal(`https://api.${process.env.HLX_PROD_SERVER_HOST_LIVE}/logout?org=adobe&site=aem-boilerplate`);
      });

      it('creates correct URL for profile endpoint', () => {
        const config = { ...siteStore, apiUpgrade: true };
        const url = createAdminUrl(config, 'profile');
        expect(url.toString()).to.equal(`https://api.${process.env.HLX_PROD_SERVER_HOST_LIVE}/profile?org=adobe&site=aem-boilerplate`);
      });

      it('creates correct URL for other endpoints', () => {
        const config = { ...siteStore, apiUpgrade: true };
        const url = createAdminUrl(config, 'status', '/path/to/resource');
        expect(url.toString()).to.equal(`https://api.${process.env.HLX_PROD_SERVER_HOST_LIVE}/adobe/sites/aem-boilerplate/status/path/to/resource`);
      });
    });
  });
  ...
  describe('callAdmin', () => {
    ...
    it('calls admin api', async () => {
      ...
      expect(url).to.equal(`https://admin.${process.env.HLX_PROD_SERVER_HOST_PAGE}/preview/adobe/aem-boilerplate/main/path/to/resource`);
    ...
    it('calls admin api with default path', async () => {
      ...
      const url = fetchStub.getCall(0).args[0].toString();
      expect(url).to.equal(`https://admin.${process.env.HLX_PROD_SERVER_HOST_PAGE}/preview/adobe/aem-boilerplate/main`);
    });
    ...
    it('calls admin api with job endpoint and apiUpgrade', async () => {
      ...
      const url = fetchStub.getCall(0).args[0].toString();
      expect(url).to.equal(`https://api.${process.env.HLX_PROD_SERVER_HOST_LIVE}/adobe/sites/aem-boilerplate/jobs/path/to/resource`);
    });
